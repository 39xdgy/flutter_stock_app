...
import 'package:fortune_cookie/themes/stock_theme.dart' as theme;
...
class _GlitchyPriceState extends State<GlitchyPrice>
  ...
  void didUpdateWidget(GlitchyPrice oldWidget) {
    ...
    if (widget.price != oldWidget.price) {
      if (double.parse(widget.price) < double.parse(oldWidget.price)) {
  ...
  Widget build(BuildContext context) {
    final currencyText = widget.price;
    ...
    return Positioned(
      ...
      child: Text(
        ...
        style: widget.textStyle
            .copyWith(color: _controller.color.withOpacity(0.4)),
      ),
    );
  }
...
class _PriceGlitchController with ChangeNotifier {
  ...
  void _onTick(Duration elapsedTime) {
    ...
    _leftGlitchPercent =
        _calculateJitterPercent(dt, _leftGlitchJitterAdjustment);
    _rightGlitchPercent =
        _calculateJitterPercent(dt, _rightGlitchJitterAdjustment);
  ...
  double _calculateJitterPercent(Duration dt, double jitterPhaseAdjustment) {
    ...
    final jitterMotion =
        sin(cyclePercent * 3 * jitterPhaseAdjustment * pi + (pi / 4));
